...
export const RefNetworkImage = ({ viewModel }) => {
  return (
  <div id="ref_network_image_page" className="w-full h-auto flex flex-row items-stretch">
    <ReferenceSidebar className="flex flex-col" />
    <div id="ref_network_image_scroll" className="w-full h-auto overflow-y-auto flex flex-col">
      <div id="ref_network_image_content" className="w-full py-12 px-6 bg-[#FFFFFF] flex flex-col">
        <span className="mb-4 text-[#23272F] text-[32px] font-bold font-bold">{StringManager.currentLanguage.refNetworkImageTitle}</span>
        <span className="mb-6 text-[#4B5563] text-base">{StringManager.currentLanguage.refNetworkImageDesc}</span>
        ...
        <div className="mb-8 flex flex-col">
          <span className="mb-2 text-[#4B5563] text-sm">{StringManager.currentLanguage.compNetworkImageUikit}</span>
          <span className="text-[#4B5563] text-sm">{StringManager.currentLanguage.compNetworkImageSwiftui}</span>
        </div>
        ...
        <div className="w-full rounded-lg border border-[#E5E7EB] overflow-hidden flex flex-col">
          ...
          {viewModel.networkImageAttributes?.map((item, index) => (
        ...
        <CodeBlock className="flex flex-col" file="network_image_example.json" language="json" showPreview={false} />
      </div>
    </div>
  </div>
  );
...
export default RefNetworkImage;
