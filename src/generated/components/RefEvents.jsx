"use client";

// Generated by ReactJsonUI - Do not edit directly
import React from 'react';
import { StringManager } from '@/generated/StringManager';
import { ReferenceSidebar } from '@/components/extensions/ReferenceSidebar';
import { CodeBlock } from '@/components/extensions/CodeBlock';


export const RefEvents = ({ data }) => {
  return (
  <div id="ref_events_page" className="w-full h-auto flex flex-row items-stretch">
    <div className="" />
    <ReferenceSidebar className="flex flex-col" />
    <div id="ref_events_scroll" className="w-full h-auto overflow-y-auto flex flex-col">
      <div id="ref_events_content" className="w-full py-12 px-6 bg-[#FFFFFF] flex flex-col">
        <span className="mb-2 text-[#23272F] text-[32px] font-bold">{StringManager.currentLanguage.refEvents}</span>
        <span className="mb-8 text-[#4B5563] text-base">{StringManager.currentLanguage.refEventsOverview}</span>
        <span className="mb-3 text-[#23272F] text-xl font-bold">{StringManager.currentLanguage.refEventsOnclick}</span>
        <span className="mb-4 text-[#4B5563] text-sm">Trigger action when component is clicked</span>
        <CodeBlock className="flex flex-col mb-6" file="event_onclick.json" language="json" showPreview={false} />
        <span className="mb-3 text-[#23272F] text-xl font-bold">{StringManager.currentLanguage.refEventsOnchange}</span>
        <span className="mb-4 text-[#4B5563] text-sm">Trigger action when value changes</span>
        <CodeBlock className="flex flex-col" file="event_onchange.json" language="json" showPreview={false} />
      </div>
    </div>
  </div>
  );
};

export default RefEvents;
