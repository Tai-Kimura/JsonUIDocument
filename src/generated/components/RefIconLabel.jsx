...
export const RefIconLabel = ({ viewModel }) => {
  return (
  <div id="ref_icon_label_page" className="w-full h-auto flex flex-row items-stretch">
    <ReferenceSidebar className="flex flex-col" />
    <div id="ref_icon_label_scroll" className="w-full h-auto overflow-y-auto flex flex-col">
      <div id="ref_icon_label_content" className="w-full py-12 px-6 bg-[#FFFFFF] flex flex-col">
        <span className="mb-4 text-[#23272F] text-[32px] font-bold font-bold">{StringManager.currentLanguage.refIconLabelTitle}</span>
        <span className="mb-6 text-[#4B5563] text-base">{StringManager.currentLanguage.refIconLabelDesc}</span>
        ...
        <div className="mb-8 flex flex-col">
          <span className="mb-2 text-[#4B5563] text-sm">{StringManager.currentLanguage.compIconLabelUikit}</span>
          <span className="text-[#4B5563] text-sm">{StringManager.currentLanguage.compIconLabelSwiftui}</span>
        </div>
        ...
        <div className="w-full rounded-lg border border-[#E5E7EB] overflow-hidden flex flex-col">
          ...
          {viewModel.iconLabelAttributes?.map((item, index) => (
        ...
        <CodeBlock className="flex flex-col" file="icon_label_example.json" language="json" showPreview={false} />
      </div>
    </div>
  </div>
  );
