...
export const RefCircleImage = ({ viewModel }) => {
  return (
  <div id="ref_circle_image_page" className="w-full h-auto flex flex-row items-stretch">
    <ReferenceSidebar className="flex flex-col" />
    <div id="ref_circle_image_scroll" className="w-full h-auto overflow-y-auto flex flex-col">
      <div id="ref_circle_image_content" className="w-full py-12 px-6 bg-[#FFFFFF] flex flex-col">
        <span className="mb-4 text-[#23272F] text-[32px] font-bold font-bold">{StringManager.currentLanguage.refCircleImageTitle}</span>
        <span className="mb-6 text-[#4B5563] text-base">{StringManager.currentLanguage.refCircleImageDesc}</span>
        ...
        <div className="mb-8 flex flex-col">
          <span className="mb-2 text-[#4B5563] text-sm">{StringManager.currentLanguage.compCircleImageUikit}</span>
          <span className="text-[#4B5563] text-sm">{StringManager.currentLanguage.compCircleImageSwiftui}</span>
        </div>
        ...
        <div className="w-full rounded-lg border border-[#E5E7EB] overflow-hidden flex flex-col">
          ...
          {viewModel.circleImageAttributes?.map((item, index) => (
        ...
        <CodeBlock className="flex flex-col" file="circle_image_example.json" language="json" showPreview={false} />
      </div>
    </div>
  </div>
  );
