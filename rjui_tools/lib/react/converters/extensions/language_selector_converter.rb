...
module RjuiTools
  module React
    module Converters
      module Extensions
        class LanguageSelectorConverter < BaseConverter
          def convert(indent = 2)
            ...
            if is_container
              ...
              <<~JSX.chomp
                #{indent_str(indent)}<LanguageSelector#{id_attr} className="#{class_name}"#{props_str}>
                #{children_jsx}
                #{indent_str(indent)}</LanguageSelector>
              JSX
            else
              <<~JSX.chomp
                #{indent_str(indent)}<LanguageSelector#{id_attr} className="#{class_name}"#{props_str} />
