...
export const Search: React.FC<SearchProps> = ({
  ...
  //
  // Hydration safety: StringManager reads localStorage in its constructor, so
  // the singleton's `getString` returns Japanese on the very first client
  // render when the user has previously toggled language — but the server
  // rendered with English. Resolve through useState/useEffect so initial
  // render matches SSR, then swap to the localized string after mount.
  ...
  useEffect(() => {
    ...
    setResolvedPlaceholder(
      StringManager.getString("home_search_placeholder") || DEFAULT_PLACEHOLDER,
    );
  }, [placeholder]);
