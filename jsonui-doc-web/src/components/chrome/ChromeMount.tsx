...
export function ChromeMount() {
  ...
  // Same deferral for the StringManager language. The singleton reads
  // localStorage in its constructor, so on the client its `.language`
  // getter already reflects the persisted value at module-load time —
  // reading it during the VM constructor would push a divergent
  // currentLanguage into the first render and break hydration on
  // TopBar's language toggle. mountLanguage applies the real value
  // post-mount so SSR and first client render both see "en".
  useEffect(() => {
