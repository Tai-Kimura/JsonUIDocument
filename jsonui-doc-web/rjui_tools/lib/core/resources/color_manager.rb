    ...
    module Resources
      class ColorManager
        ...
        def process_colors(processed_files, processed_count, skipped_count, config)
          ...
          # Generate ColorManager.js if needed
          generate_color_manager_js if @config['generated_directory']
        end
        ...
        # Generate JavaScript/TypeScript code for ColorManager
        def generate_color_manager_js
          ...
          output_file = File.join(generated_dir, 'ColorManager.js')
          ...
          js_code = generate_js_code(all_colors)

          File.write(output_file, js_code)
          Core::Logger.info "✓ Generated ColorManager.js"
        end

        def generate_js_code(colors)
          ...
          code << ''
          code << 'class ColorManagerClass {'
          code << '  constructor() {'
          code << '    this._colors = colorsData;'
          code << '  }'
          code << ''
          code << '  /**'
          code << '   * Get color value by key'
          code << '   * Returns undefined for binding expressions like @{...} without logging'
          code << '   * Logs warning if color key is not found'
          code << '   * @param {string} key - Color key from colors.json'
          code << '   * @returns {string|undefined} Hex color value or undefined'
          code << '   */'
          code << '  color(key) {'
          code << '    // Skip binding expressions - return undefined without logging'
          code << '    if (key && key.startsWith("@{") && key.endsWith("}")) {'
          code << '      return undefined;'
          code << '    }'
          code << ''
          code << '    // Check if color exists'
          code << '    if (this._colors[key]) {'
          code << '      return this._colors[key];'
          code << '    }'
          code << ''
          code << '    // Try to parse key as hex color if not found'
          code << '    if (this.isHexColor(key)) {'
          code << '      return key;'
          code << '    }'
          code << ''
          code << '    // Log warning for missing color'
          code << '    console.warn(`[ColorManager] Warning: Color key "${key}" not found in colors.json`);'
          code << '    return undefined;'
          code << '  }'
          code << ''
          code << '  /**'
          code << '   * Check if a string is a valid hex color'
          code << '   * @param {string} value'
          code << '   * @returns {boolean}'
          code << '   */'
          code << '  isHexColor(value) {'
          code << '    if (typeof value !== "string") return false;'
          code << '    return /^#?([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$/.test(value);'
          code << '  }'
          code << ''
          code << '  /**'
          code << '   * Get all available color keys'
          code << '   * @returns {string[]} Array of color keys'
          code << '   */'
          code << '  get availableColors() {'
          code << '    return Object.keys(this._colors);'
          code << '  }'
          code << ''

          # Generate static color accessors (camelCase)
          unless colors.empty?
            code << '  // Static color accessors (camelCase)'
            colors.keys.sort.each do |key|
              property_name = snake_to_camel(key)

              if @colors_data[key]
                code << "  get #{property_name}() { return \"#{@colors_data[key]}\"; }"
              else
                code << "  get #{property_name}() {"
                code << "    console.warn('[ColorManager] Warning: Color \\'#{key}\\' is not defined in colors.json');"
                code << "    return undefined;"
                code << "  }"
              end
            end
          ...
          code << '}'
          code << ''
          code << 'export const ColorManager = new ColorManagerClass();'
          code << 'export default ColorManager;'
