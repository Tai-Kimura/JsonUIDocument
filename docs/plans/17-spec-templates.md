...
- 独自コンポーネント（`CodeBlock` / `NavLink` 等）は `.jsonui-doc-rules.json` に登録済みとする
...
- Layout では `Tabs` / `TabPanel` で UIKit / SwiftUI の切替（Swift）、Compose / XML の切替（Kotlin）を表現

```jsonc
{
  "type": "Tabs",
  "tabs": [
    { "label": "UIKit",   "panel": "uikit_panel" },
    { "label": "SwiftUI", "panel": "swiftui_panel" }
  ],
  "child": [
    { "type": "TabPanel", "id": "uikit_panel",   "child": [ /* UIKit 解説 */ ] },
    { "type": "TabPanel", "id": "swiftui_panel", "child": [ /* SwiftUI 解説 */ ] }
  ]
}
```

`Tabs` は全プラットフォーム対応 Converter。
