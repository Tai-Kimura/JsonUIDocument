...
- [ ] 独自 component_spec を作成（spec-first。`02-tech-stack.md §6.1`）
  - `docs/screens/json/components/codeblock.component.json`
  - `docs/screens/json/components/table_of_contents.component.json`（Web 専用）
  - `docs/screens/json/components/search_modal.component.json`（Web 専用）
  - `docs/screens/json/components/search_trigger.component.json`（Web 専用）
  - `docs/screens/json/components/open_api_viewer.component.json`（Web 専用）
  - `docs/screens/json/components/json_schema_viewer.component.json`（Web 専用）
- [ ] 各 spec で `props.items[]`（name / type）と `slots.items[]`（container の場合は非空）を定義し、`mcp__jui-tools__doc_validate_component` で検証
- [ ] `.jsonui-doc-rules.json` の `componentTypes.screen` に上記 6 種の型名を登録（`15-toolchain.md §2.3`）
- [ ] Converter 生成（spec-driven。`--attributes` を手書きしない）
  - `jui g converter --from docs/screens/json/components/codeblock.component.json`
  - 他 5 件も同様。または `jui g converter --all` で一括生成
- [ ] Converter 本体を実装（Shiki、FlexSearch、Redoc、JSON Schema レンダラ）
- [ ] ナビゲーション・タブ・バッジ・prose ラッパは**標準コンポーネント + ViewModel バインディング + `cells/*`** で表現する方針（`02-tech-stack.md §6.0`）。個別カスタム Converter は作らない
...
- [ ] Claude / Codex 差分表示は ViewModel `activeAgent` + `visibility` バインディング（`02-tech-stack.md §6.0`）。タブヘッダは `Collection + cells/tab_header`
...
- [ ] 独自 Converter（`CodeBlock`）の iOS / Android 実装（`TableOfContents` / `SearchModal` / `SearchTrigger` / `OpenApiViewer` / `JsonSchemaViewer` は Web 専用）
...
- [ ] `/learn/installation` 最優先実装（`18-content-plan-installation.md`）。ホーム Hero の第一 CTA にもなるため Phase 6 の先頭で着手。`/tools/{cli,mcp,agents}/install` からもバナーでリンク
- [ ] `/learn/` 配下 残り（`what-is-jsonui` / `hello-world` / `first-screen` / `data-binding-basics` / `next-steps`）
- [ ] `/guides/` 配下（タスク別）
...
| Converter（`CodeBlock`）の全プラットフォーム実装負担 | 中 | Web を先行、iOS/Android はプレースホルダで OK。段階的に改善 |
...
- 独自 Converter 群（`CodeBlock` / `TableOfContents` / `SearchModal` / `SearchTrigger` / `OpenApiViewer` / `JsonSchemaViewer`）は upstream にコントリビュート
