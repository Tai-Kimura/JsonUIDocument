...
- プラットフォーム対応マトリクスは `PlatformBadge` × 5
...
導線は **TabView + include されたサイドバー + `NavLink`** で構成。`NavLink` は Web 専用 Converter で、`href` / `activeClass` / `exact` 属性を持つ。
...
- **アクティブ判定**（Web）: Next.js の `usePathname` を `NavLink` Converter が利用（`activeClass` 属性）
