...
**「Sumi × Cinnabar × Kindei」**: warm-neutral dark foundation + brand blue 保持 + cinnabar（朱）副次アクセント + kindei（金泥）リンク色（dark mode 限定）。古典的な sumi × 朱 × 金の 3 色トライアドに対応。
...
2. Brand blue `#2563EB` は light 専用。dark は kindei gold にフル統一（light で使っているチュートリアル画像との整合性は screenshot 自体が light テーマ撮影のため保持）
...
| token | light | dark | 用途 |
|---|---|---|---|
| `accent` | `#2563EB` | `#D4A574` | primary button bg。dark は kindei gold（sumi × 金 aesthetic に完全統一、brand blue は light 専用） |
| `accent_hover` | `#1D4ED8` | `#C79060` | hover。dark は deeper kindei/copper |
| `accent_ink` | `#FFFFFF` | `#1A1612` | accent bg 上の text。dark は warm ink（gold 上に sumi 文字 = 金箔に墨書） |
| `accent_link` | `#2563EB` | `#D4A574` | **金泥（kindei gold）**：dark mode のリンク text 色。`#2563EB` は dark surface で AA fail、`#60A5FA`（blue-400）は warm-neutral surface 上で冷たく浮く。古典的な sumi × 朱 × 金 トライアドで 3 色目の金を採用。contrast 9.5:1 AAA |
| `accent_marker` | `#2563EB` | `#D4A574` | 番号バッジ / role バッジ / step 円の **背景**。`accent_link` と同じ色遷移（青→金）だが、役割は「bg 上の pure_white 文字」。`accent_link` は text-link 用なので bg として流用すると意味が混乱するため別トークンに分割 |
| `accent_tint` | `#EEF2FF` | `#2B2418` | 選択行 bg（sidebar active / Search active-row / binding chip bg）。dark は warm sumi ink — `accent_link` gold と合わせて sumi × kindei の選択ハイライト |

### 2.4 Accent secondary（light: indigo / dark: bronze、カテゴリアイコン・binding chip・情報ボックス）

| token | light | dark | 用途 |
|---|---|---|---|
| `accent_secondary` | `#6366F1` | `#C89266` | カテゴリアイコン fill。dark は bronze（kindei gold と区別する warm metal） |
| `accent_secondary_strong` | `#4F46E5` | `#E0A870` | binding chip / reference callout の **text**。dark は copper amber |
| `accent_secondary_tint` | `#E0E7FF` | `#2E2418` | chip / callout の **bg**。dark は warm ink（`accent_tint` 近傍） |

**なぜ dark で indigo → bronze か**: 初版は indigo で揃えたが、dark mode で「info callout が全面 indigo」に見えて sumi × kindei の暖色アクセントから孤立していた。bronze 系に寄せることで primary kindei gold とは別の warm metal として差別化しつつ、全体を一貫した warm neutral で統一。
...
| dark **accent_link** `#D4A574` / dark surface_muted | 9.5 : 1 | AAA |
...
| 2026-04-23 | `accent_marker` 追加 / `accent_tint` dark を `#2B2418` warm sumi にシフト | step 円バッジ等の dark mode gold 化 + sidebar active の gold-on-sumi 化 |
| 2026-04-23 | `accent_secondary_*` dark を indigo → bronze にシフト | info callout / binding chip / カテゴリアイコンの青基調を解消 |
| 2026-04-23 | `accent` dark を `#2563EB` → `#D4A574` kindei gold に / `accent_ink` dark を `#1A1612` warm ink に / `accent_hover` dark を `#C79060` に / `eyebrow` dark を `#E0A870` copper に / `accent_tap` dark を `#3D2F1A` warm inkstone に | dark mode を brand blue 含め完全 warm 化。primary button は gold bg + sumi ink text（金箔に墨書）の古典 aesthetic |
