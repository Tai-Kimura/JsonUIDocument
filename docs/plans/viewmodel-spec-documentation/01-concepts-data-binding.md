---
title: "Plan 01 — Data binding as contract ViewModel documentation"
status: open
depends_on: [00-overview]
spec: docs/screens/json/concepts/data-binding.spec.json
shape: minimal
owner: jsonui-define
---

# Plan 01 — Data binding as contract
...
Add `dataFlow.viewModel` using the **minimal** template from [00-overview.md](00-overview.md). String-key prefix for this screen: **`concepts_data_binding_`**.
...
- `NEXT_READ_ENTRIES` catalog (authoritative mapping from the real VM body):
  ...
  |---|---|---|
  | `next_why_spec_first` | `/concepts/why-spec-first` | `concepts_data_binding_next_why_spec_first_title` + `concepts_data_binding_next_why_spec_first_description` |
  | `next_viewmodel` | `/concepts/viewmodel-owned-state` | `concepts_data_binding_next_viewmodel_title` + `concepts_data_binding_next_viewmodel_description` |

Open `jsonui-doc-web/src/viewmodels/concepts/DataBindingViewModel.ts` to confirm the exact entries currently seeded. Document what you find; do not invent new entries here.
