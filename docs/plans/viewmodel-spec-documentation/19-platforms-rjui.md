---
title: "Plan 19 — ReactJsonUI ViewModel documentation"
status: open
depends_on: [00-overview]
spec: docs/screens/json/platforms/rjui.spec.json
shape: minimal
owner: jsonui-define
---
...
Add `dataFlow.viewModel` using the **minimal** template from [00-overview.md](00-overview.md). String-key prefix for this screen: **`platforms_rjui_`**.

### Page-specific concerns

- Navigation destinations (listed in `transitions`, repeat here for the VM method description): `/platforms/kotlin`, `/platforms/swift`
- `NEXT_READ_ENTRIES` catalog (authoritative mapping from the real VM body):
  | id | url | strings.json keys (title, description) |
  |---|---|---|
  | `next_swift` | `/platforms/swift` | `platforms_rjui_next_swift_title` + `platforms_rjui_next_swift_description` |
  | `next_kotlin` | `/platforms/kotlin` | `platforms_rjui_next_kotlin_title` + `platforms_rjui_next_kotlin_description` |

Open `jsonui-doc-web/src/viewmodels/platforms/RjuiViewModel.ts` to confirm the exact entries currently seeded. Document what you find; do not invent new entries here.
