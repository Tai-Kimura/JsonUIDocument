---
title: "Plan 12 — Data binding basics ViewModel documentation"
status: open
depends_on: [00-overview]
spec: docs/screens/json/learn/data-binding-basics.spec.json
shape: minimal
owner: jsonui-define
---

# Plan 12 — Data binding basics
...
Add `dataFlow.viewModel` using the **minimal** template from [00-overview.md](00-overview.md). String-key prefix for this screen: **`learn_data_binding_basics_`**.

### Page-specific concerns

- Navigation destinations (listed in `transitions`, repeat here for the VM method description): `/concepts/data-binding`, `/learn/first-screen`
- `NEXT_READ_ENTRIES` catalog (authoritative mapping from the real VM body):
  | id | url | strings.json keys (title, description) |
  |---|---|---|
  | `next_concepts_data_binding` | `/concepts/data-binding` | `learn_data_binding_basics_next_concepts_data_binding_title` + `learn_data_binding_basics_next_concepts_data_binding_description` |
  | `next_first_screen` | `/learn/first-screen` | `learn_data_binding_basics_next_first_screen_title` + `learn_data_binding_basics_next_first_screen_description` |

Open `jsonui-doc-web/src/viewmodels/learn/DataBindingBasicsViewModel.ts` to confirm the exact entries currently seeded. Document what you find; do not invent new entries here.
