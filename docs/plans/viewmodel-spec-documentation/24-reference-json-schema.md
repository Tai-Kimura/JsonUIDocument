---
title: "Plan 24 — JSON Schema ViewModel documentation"
status: open
depends_on: [00-overview]
spec: docs/screens/json/reference/json-schema.spec.json
shape: minimal
owner: jsonui-define
---
...
Add `dataFlow.viewModel` using the **minimal** template from [00-overview.md](00-overview.md). String-key prefix for this screen: **`reference_json_schema_`**.

### Page-specific concerns

- Navigation destinations (listed in `transitions`, repeat here for the VM method description): `/concepts/why-spec-first`, `/reference/attributes`
- `NEXT_READ_ENTRIES` catalog (authoritative mapping from the real VM body):
  | id | url | strings.json keys (title, description) |
  |---|---|---|
  | `next_attributes` | `/reference/attributes` | `reference_json_schema_next_attributes_title` + `reference_json_schema_next_attributes_description` |
  | `next_spec_first` | `/concepts/why-spec-first` | `reference_json_schema_next_spec_first_title` + `reference_json_schema_next_spec_first_description` |

Open `jsonui-doc-web/src/viewmodels/reference/JsonSchemaViewModel.ts` to confirm the exact entries currently seeded. Document what you find; do not invent new entries here.
