---
title: "Plan 06 — Building a custom component ViewModel documentation"
status: open
depends_on: [00-overview]
spec: docs/screens/json/guides/custom-components.spec.json
shape: minimal
owner: jsonui-define
---

# Plan 06 — Building a custom component
...
Add `dataFlow.viewModel` using the **minimal** template from [00-overview.md](00-overview.md). String-key prefix for this screen: **`guides_custom_components_`**.

### Page-specific concerns

- Navigation destinations (listed in `transitions`, repeat here for the VM method description): `/concepts/why-spec-first`, `/guides/writing-your-first-spec`
- `NEXT_READ_ENTRIES` catalog (authoritative mapping from the real VM body):
  | id | url | strings.json keys (title, description) |
  |---|---|---|
  | `next_spec_first` | `/guides/writing-your-first-spec` | `guides_custom_components_next_spec_first_title` + `guides_custom_components_next_spec_first_description` |
  | `next_concepts_spec` | `/concepts/why-spec-first` | `guides_custom_components_next_concepts_spec_title` + `guides_custom_components_next_concepts_spec_description` |

Open `jsonui-doc-web/src/viewmodels/guides/CustomComponentsViewModel.ts` to confirm the exact entries currently seeded. Document what you find; do not invent new entries here.
