...
ドキュメントサイト固有のコンポーネントタイプを各プラットフォームの validator に認識させる名前ホワイトリスト。列挙する型は **`docs/screens/json/components/*.component.json` が先に存在している** ことが前提（spec-first。`02-tech-stack.md §6.1`）。
...
> ナビゲーションリンク（旧 `NavLink`）、同ページ内タブ（旧 `Tabs`/`TabPanel`）、プラットフォームバッジ（旧 `PlatformBadge`）、リッチテキストラッパ（旧 `Prose`）は**標準コンポーネント + ViewModel バインディング + `cells/*`** で表現するため、ここには含めない（`02-tech-stack.md §6.0`）。
