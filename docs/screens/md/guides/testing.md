...
- 2026-09-03 - section_cli_bullet_validate gains the 1.8.20 selectOption warning: a step carrying two or more of index / value / label is warned about, not failed, with the precedence spelled out. Measured on a scratch test file with value + label: 1.8.19 prints nothing, 1.8.20 prints the quoted [WARN] line and 'Warnings: 1' with Result: PASSED; a control step with value alone prints nothing. The message is quoted verbatim.
- 2026-09-04, version-drift audit asked by the upstream lane, answered by measurement: this site pins no library or driver version anywhere in its prose. Of 413 version numbers, 24 are minimum-version floors (X.Y.Z+) that stay true as versions rise, and the shapes that CAN go stale - a claim that some version is current, or a pinned dependency line - are zero, with the detectors run against planted text first so a zero is not a dead pattern. The convention is stated on the page itself: the drivers are named by registry (npm @latest, Maven Central 'pin the latest', SPM from:) and one sentence says the registry, not this page, is where the version comes from. That is why the web driver moving to 1.8.6 today changes nothing here. Worth recording how the audit went wrong first: a classifier that split historical from current claims by looking at the words immediately before the number reported 345 suspects, because 'since jsonui-cli 1.8.5' puts a package name between the marker and the version. The broken classifier was discarded rather than reported, and the question narrowed to the shapes that can actually rot.
