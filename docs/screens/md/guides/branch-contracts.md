...
- 2026-09-08 (2) - jsonui-cli 1.8.50 corrects a word this page published the same day. The ambiguity bullet said a candidate owner is one that declares `an HTTP endpoint`; the predicate is a SHAPE — `^([A-Z]+)\s+(\S+)$` — so a non-HTTP route written that way (`"RTDB onValue(rooms/{id})"` parses as method `RTDB`) lands on the same shelf as `GET /x` and can make a name ambiguous too. The wording was not read off the behaviour: it was read off the internal name `_declares_http_endpoint`, and 1.8.50 renames that to `_declares_bindable_endpoint` for exactly this reason, upstream having recorded that a reader took the old name at its word within a day. That reader was this page. The bullet now says `bindable`, states the shape, gives the non-HTTP example, and keeps the correction visible rather than silently swapping the adjective — a rename inside a tool looks like an internal change and turns out to reach published prose, which is worth one sentence to whoever edits this next. Nothing else on this face moved: the release is a no-op here, measured before it landed as an A/B of the shipped trees over the real corpus — 104 specs, `validate spec` byte-identical, and `generate spec` in both formats byte-identical (104 html + 104 md) and still equal to the committed tree. The release's headline change (unit-target pages get a resolving back-link to their app's spec page) cannot reach this face at all: it lives in `_pre_generate_spec_docs`, which only `generate html` calls, and this repository invokes `generate html` nowhere — its only entry point is `generate spec`. Its denominator here is 0 besides: no spec declares a `unitContracts` block, the three files matching that string being prose. Second release running where a `test_doc/generator.py` change was announced as affecting this face and did not; upstream has accepted writing future notices by the command a face runs rather than by the file that changed.
- 2026-09-25 — v1.8.117 uptake: section 9 gains a paragraph and a link to the new Contract gaps guide (user direction: the feature gets its own page and sidebar entry rather than a section here). The paragraph claims only what that page measures. Nothing in this page's declaration vocabulary changed between 1.8.110 and 1.8.117 apart from the new declaration sites, which are documented there.
- 2026-09-25 — v1.8.118 uptake, and two sentences that were stale at the previous pin. The when section said 'four key shapes' and its table listed four, but the validator has listed five since state.<name> arrived in 1.7.29 (this page documents state.<name> in the then section); 1.8.118 adds harness.<name>, so the section now says six and the table carries both. The validate transcript's unknown-key line quoted the four-key allow-list; under 1.8.117 the tool already printed five, and under 1.8.118 it prints six with glosses — re-pasted from 1.8.118. The other seven lines of that transcript were re-measured by seeding the same mistakes into one spec: identical under 1.8.117 and 1.8.118, and matching the page.
