...
- 2026-09-25 — section_declare_body: unitContracts also lives in the app contracts spec (type app_contracts_spec), where it is required because apiOutcomeRules names its cases in verifiedBy (validate refuses a rules-only file with `Required field 'unitContracts' is missing`). Measured: `generate unit-stubs --check` counts a case declared only there — '1 case(s) declared across 1 spec file(s) scanned (1 spec file(s) carrying a unitContracts block)' — with jui.config.json's platforms given as an object. With platforms given as a list, which contracts coverage accepts, generate unit-stubs raised AttributeError; that is reported upstream rather than documented here.
- 2026-09-25 — 1.8.118 fixed the crash noted above: with platforms written as a list, generate unit-stubs --check now reports each platform as NOT CHECKED, naming the list shape as the reason ('which has no place for unitTestsDir'), where 1.8.117 raised AttributeError. Measured on the same fixture under both versions. Nothing on the page described the crash, so no page text changes.
- 2026-09-25 — v1.8.119: unitContracts in the app contracts spec is required only beside apiOutcomeRules. Measured: a harnessConditions-only app spec fails under 1.8.118 with `Required field 'unitContracts' is missing` and passes under 1.8.119; an apiOutcomeRules file without unitContracts still fails.
