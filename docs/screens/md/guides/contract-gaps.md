...
- 2026-09-25 — v1.8.119 uptake. Every fixture behind the page was re-run under 1.8.118 and 1.8.119 and diffed. Coverage prints two more lines per block, `baselined …` and `data (report only) …`, so the three quoted coverage blocks and the 409 test (now installFetchMock with apiOriginsOf, and reportUnmatched after the act) are re-pasted. The largest correction is section 5. The page said a rule changes nothing when the screen does not route the side call, and 1.8.119 made that false: the side call is served as a side route and admitted only in tests serving the rule's statuses (measured on the 401 row without the logout route: side_calls_admitted 0 at 1.8.118, 1 at 1.8.119; allowance postLogout+updateProfile in the 401 test, updateProfile elsewhere). unitContracts is now required only beside apiOutcomeRules: an app spec with only harnessConditions fails under 1.8.118 and passes under 1.8.119, and one with rules and no unitContracts still fails. Section 8 is new — validate's coverage section and `contracts baseline` — measured on the example under --no-mock-check: validate prints the per-platform line and the 'from jsonui-cli 1.8.120' line with exit 0; baseline refuses without --initial, writes 3 entries with it, reports stale 1 after the 422 row lands, and removes 3 on the next run. --no-mock-check was used because validate's mock check rewrites mocks the fixture had placed by hand under generated/ (the same under 1.8.118; the generator owns generated/default/<op>.mock.json). This is a fixture choice, not a finding.
- 2026-09-25 — v1.8.120 (hotfix) uptake. The coverage gate moved from 1.8.120 to 1.8.121. The page named 1.8.120 in four strings and in the §8 transcript, which is what the 1.8.119 pin printed; all now name 1.8.121, taken from the 1.8.120 validate output ('from jsonui-cli 1.8.121, validate fails …'). §8 says the gate was moved on. Coverage output did not move across 1.8.119 → 1.8.120 on any page fixture. The generated 409 test is re-pasted: reportUnmatched now carries the row's name. §5 gains not-called for a side call, measured on the failure row with postLogout undeclared: 1.8.119 stops generation with 'has no endpoint declaration'; 1.8.120 generates, and the row and its 409 copy assert 'api.postLogout: this row says not-called — called N time(s)'.
