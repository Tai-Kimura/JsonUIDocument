...
- 2026-09-25 — v1.8.117 uptake: section 6 rewritten from the shipped tool rather than the release notices. Measured on fresh three-platform projects (jui init at 1.8.117, SwiftUI and Compose modes): from a spec that declares props, `jui g converter --from` writes 18 files — per platform the three tool-side files under extensions/ (converter, attribute_definitions/<Name>.json, mappings) plus the app-side stubs (web 1, iOS 3, Android 5); from a spec with no props attribute_definitions is not written (15 files). Re-runs measured by marking every written file: no flag with stdin closed, and --skip-existing, both exit 0 and keep 15 of 18 marks (the three attribute_definitions files are rewritten on every run); --force replaces 11 (converters, stubs, attribute_definitions) and keeps the 7 mappings and registration files. At 1.8.110 the same re-run with no terminal exited 1, and so did --skip-existing, because the iOS and Android stub prompts failed on a closed stdin — so the old sentence ('--skip-existing … exits 0 instead of erroring on conflict') was already false on a three-platform project at the previous pin. The Android file names (<name>_component.rb, component_mappings.rb) and the --force hazard for stubs you have written into are added.
