...
- Ten numbered steps cover: (0) prep / read a neighbour spec, (1) name + platform + PascalCase, (2) uiVariables + free-form initial literal, (3) eventHandlers + onX pattern, (4) customTypes, (5a) jsonui-doc validate spec sub-command, (5b) jui generate project incl. --dry-run / --web-only and hand-edit protection, (5c) jui build + jui verify --fail-on-diff with three drift kinds (structural / orphan data / unregistered custom types), (6) @generated vs hand-written file tree. Each carries one CodeBlock.
...
- The guide's subject matter IS the framework's core flow (author spec → jsonui-doc validate → jui generate → jui build → jui verify → iterate). Treating it as the Guides tab's keystone entry makes 'start here' obvious to a reader who already knows what JsonUI is but hasn't written one yet.
...
- 2026-09-11 - Correction: the steps above and the flow sentence named `jui doc validate`, and the guide itself told readers to run `jui doc validate spec --file <path>`. There is no `doc` subcommand under `jui` — it exits 2 with `invalid choice: 'doc'` — and the real command takes the file as a POSITIONAL argument, so `--file` fails a second time with `unrecognized arguments: --file`. Both were wrong for as long as the page has existed and no gate could see them: the CLI reference's cards are checked against `--help`, but a command typed inside a guide's CodeBlock was checked by nothing. Corrected to `jsonui-doc validate spec <file>` and `jsonui-doc validate component <file>`, and `jsonui-doc init component <Name>` in the custom components guide, each verified by running it: exit 0, and `init component Badge` writes docs/components/json/badge.component.json.
