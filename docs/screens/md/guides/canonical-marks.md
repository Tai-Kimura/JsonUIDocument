...
- 2026-09-03 - jsonui-cli 1.8.17 adds a NOTE line to jui build naming the arguments a mark widens from a canon enum to String (the DTO holds a generated enum, so a repository converts at the boundary). Measured here with a fixture whose mark resolves and whose body field is an inline string enum: the inventory function returns the argument (provider -> SnsLoginRequestProvider) on the raw spec and nothing after the marks are expanded in place, and the printing path evaluates it after the expansion, so jui build prints zero NOTE lines on every project at 1.8.17. The same fixture with an undeclared endpoint fails the build ('cannot be resolved'), so the expansion path itself is reached by this site's web-only build. Nothing in the page's prose depends on the NOTE line, so the text is unchanged; when the line starts printing, the expansion section is where one sentence about it belongs.
- 2026-09-03 - the expansion section gains the 1.8.18 NOTE line: one line per spec naming the arguments whose canon type is an inline enum, printed as a NOTE and not a warning. Measured before writing: this site's build prints 0 of 478 lines (no docs/api, no marks), and a synthetic fixture with a mark on an inline string enum prints exactly one - 'provider -> SnsLoginRequestProvider' - at build level and at the extractor, with a hand-written params list printing none. At 1.8.17 the same fixture printed nothing, because the inventory ran after the marks had been expanded in place; the sentence waited for the fix rather than describing a line that could not appear.
- 2026-09-04 - jsonui-cli 1.8.36 retracts a claim this page had repeated: `jui build` does not gate on warnings. Measured here on 1.8.36 in a throwaway project, one layout, five runs — an unknown attribute, a missing required attribute, an unknown component type and a `--lint-strings` localize finding are each printed and the build exits 0, while `jui lint-strings` on the same tree exits 2. The build failed only on inputs that stop the run (a layout that is not valid JSON exits 1; the code's other early returns are a missing jui.config.json and a responsive-variant contract violation). Upstream's own comment carried the same wrong wording until 1.8.36 and says a consumer read it as a mechanism. The flag's `--help` still calls it 'the localize gate', so the card now names that wording and corrects it rather than dropping it. This page's last sentence assumed the same mechanism ('a build gated at zero warnings does not fail on it') and now says neither a NOTE nor a warning changes the exit code.
