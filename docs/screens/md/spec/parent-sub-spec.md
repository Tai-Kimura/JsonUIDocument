...
- 2026-09-04 - the eleventh gate fired for the first time on a real release: jsonui-cli 1.8.30 added `validation` to MERGER_BUILDS_FROM_SUB_SPECS (11 names to 12) and the check went red naming it, before anything else here noticed. Measured where it belongs rather than assuming: it is NOT in PARENT_READS_TOP_LEVEL, so it is a section that must move to a sub-spec and belongs in the behaviour sentence; and the merger keys it inside each of its two subsections - clientSide on `field`, serverSide on `condition` - so it belongs in the merge-keying list too, with that detail. Upstream's own note is worth keeping in mind: the section had been shipping while being dropped, and it surfaced only when a face asked which of its declared sections actually arrive.
- 2026-09-04, page-convention ruling asked of this lane and answered (a): the generated page for a parent spec is an INDEX, not a merged view. Measured first, independently of the notice: document_tools imports the merger 0 times while jui_tools' build, verify, generate and repository_aggregator all do, and the HTML generator renders an explicit Sub Specifications table - so the shape is deliberate, not an oversight. The deciding argument is that a merged parent page would teach what the toolchain refuses: a reader seeing Data Flow and Validation on the parent's page opens the parent spec to find them, and declaring one there is a default-deny error. Secondary: two renderings of one declaration drift apart when only one is regenerated, which is the class this lane has spent the week on. Two conditions attached: the parent page must SAY that behaviour lives on the sub pages (absence is only legible where it is stated), and the Sub Specifications table should carry a column naming the sections each sub supplies, so the index can answer 'which page has validation?' without duplicating anything. Stated with the standing declared: this site has zero split screens, so the ruling is editorial, not experiential.
- 2026-09-04 - jsonui-cli 1.8.31 implements both conditions attached to yesterday's (a) ruling, and one premise this lane asserted while making it was wrong. I wrote that the generator 'already opens the sub files to make links, so the column can be derived from their top-level keys'; reading 1.8.30 shows it does not - the link is `sub_file.replace('.spec.json', '.html').split('/')[-1]`, pure string work, zero file reads near that table. The column needed a spec directory threaded through, which upstream added. Verified the result on a fixture built here (one parent, two subs): the Declares column lists exactly the behaviour sections each sub declares, a `notes` key planted as a decoy in one sub does NOT appear (bookkeeping keys are excluded), the parent page carries the sentence saying behaviour is declared in the subs and documented on their pages, and markdown - which had no index at all before - now carries both. Both CLI modes pass the directory, so the empty-cell fallback is not reachable from the command line; calling the generator as a library without it renders the column header with blank cells, which reads as 'declares nothing' and would be better omitted.
