...
- 2026-09-04 - jsonui-cli 1.8.36 retracts a claim this page had repeated: `jui build` does not gate on warnings. Measured here on 1.8.36 in a throwaway project, one layout, five runs — an unknown attribute, a missing required attribute, an unknown component type and a `--lint-strings` localize finding are each printed and the build exits 0, while `jui lint-strings` on the same tree exits 2. The build failed only on inputs that stop the run (a layout that is not valid JSON exits 1; the code's other early returns are a missing jui.config.json and a responsive-variant contract violation). Upstream's own comment carried the same wrong wording until 1.8.36 and says a consumer read it as a mechanism. The flag's `--help` still calls it 'the localize gate', so the card now names that wording and corrects it rather than dropping it. The warnings section is rewritten: it used to say the build is expected to run clean and that binding-grammar violations are errors that fail it.
- 2026-09-04 - the sentence removed here today ('canonical binding-grammar violations are errors that fail the build') was true of a different command. `sjui build` keeps canonical binding-rule errors separate from advisory warnings and fails on them whatever the flags, with other validation errors gated behind --strict; its card has said so all along, and the source at 1.8.36 is consistent with it (binding_validator_core.rb, and build.rb's own comment). `jui build` on the web path, measured, does not: `@{ bad name }` in a Label's text raises nothing, exits 0 and emits `${data.bad name}`, which is not valid JavaScript, while `@{unclosed` does raise one warning — the check tests the delimiters, not the contents. Filed upstream. The page now names the sjui difference so the removed claim is findable where it is actually true, and says plainly that the measurements are the web path, the only one this site runs.
- 2026-09-04 - the sjui half of yesterday's note is now measured, not read. iOS scratch project, SwiftUI mode, one Label: `@{a ?? b ?? c}` (the canonical rule binding-double-default, severity error) exits 1 with `Build failed: 1 binding error(s) (canonical severity: error)`, while `@{a ?? b}` in the same position exits 0 — and exits 0 under --strict too, carrying four binding warnings. An unknown attribute exits 0 plain and 1 under --strict (`Build failed: 1 validation error(s)`). The same unknown attribute on the web path exits 0 and has no --strict to reach for. So the two builds differ in kind, not degree, and the page's sentence about it rests on runs rather than on the neighbouring card.
- 2026-09-04 - every failure case this page names is now run, not read off the command's early returns. No jui.config.json exits 1 ('ERROR: jui.config.json not found'). A layout that is not valid JSON exits 1 on a JSONDecodeError. A responsive-variant contract violation exits 1 naming the file and the rule — measured by dropping an orphan `name@regular.json` beside a base that does not exist, with the same tree minus that file exiting 0 as the control. The unresolvable @canonical mark was measured on 2026-09-03 and is already in this ledger. The page said these three in a sentence that came from reading the source; they are measurements now.
- 2026-09-04 - jsonui-cli 1.8.38 lands two things this page had measured as broken. A binding whose contents are not an expression is now warned about and emitted as literal text: `@{ bad name }` raises one warning and the generated file parses, where at 1.8.36 it raised nothing and produced a file that did not. The warning-counting expression in the toolchain's own comment gained `| grep -vic 'warnings found'`, which is the off-by-one reported from here, and upstream documented the exclusion's own blind spot in the same block: a finding whose message contains that phrase is dropped with the summary. The `jui build` card also loses its parenthetical about the flag's help calling the scan a gate, because the help no longer does; the card is re-stamped against 1.8.38 and help churn for the release is 1 of 61 leaves, that one.
