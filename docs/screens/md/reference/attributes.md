...
- 2026-09-04 - jsonui-cli 1.8.39 makes the Segment items contract enforceable, and the page's dated clause is retired. Measured as an A/B against a 1.8.38 baseline, eight element shapes on the web face plus object and binding on iOS and Android. Object, array and null elements now cost their element and raise one warning naming the index; a binding given to the whole attribute raises its own warning and generates nothing; strings, booleans and numbers survive, and the boolean renders as `true`. At 1.8.38 every one of those shapes was silent. The A/B also showed that a binding in Segment items used to fail the iOS build outright and now exits 0 with warnings, so a project that leaned on that failure stops hearing about it; the release preview had said as much in a clause this lane misread, and the measurement confirms it rather than discovering it.
- 2026-09-05 - jsonui-cli 1.8.44 makes the item contracts uniform, so two notes here are rewritten from a three-face A/B rather than from the announcement. A Segment given a binding for `items` is refused on web, iOS and Android alike: the screen is not generated and the build exits 1, with `--allow-partial` as the opt-out. That took two releases — 1.8.43 gated web and Android but left iOS printing the same [error] line, exiting 0, and writing a screen whose whole body was `Text("Placeholder")`, which this lane found by comparing against a 1.8.42 baseline and filed. Confirmed fixed here: the iOS view is now absent entirely. Collection's cellClasses gains the rule the declaration started stating in 1.8.43 — one cellClass with `items` renders every item, several need `sections[].cell`, and declaring several without sections is refused with `only the first would render`. At 1.8.42 that same layout built silently and shipped a Collection using only its first cell class.
- 2026-09-07 - jsonui-cli 1.8.46 makes a bound colour inside `partialAttributes` resolve at runtime on the web face, and the Label note now says so. Measured as an A/B of the shipped 1.8.45 and 1.8.46 trees over one fixture: at 1.8.45 `fontColor: "@{accent}"` was pasted into a Tailwind class name, `text-@{accent}`, and the build warned that the colour was not defined for every mode in colors.json, reading the binding as if it were a palette key; at 1.8.46 the same input compiles to `style: { color: ColorManager.resolveColor(data.accent) }` and that warning is gone. Two earlier arms had to be thrown away first, and neither failed — both reported the two versions as agreeing. The first exported JSONUI_CLI_PATH and took that for a choice of toolchain. It is not one: only `sync_tool` reads that variable, while `jui build` resolves the web generator by walking up from the build directory for a vendored `rjui_tools` and otherwise falling back to the install on PATH, which auto-updates. Both arms had run the same toolchain, and its version was the newer one. The second vendored the tag tree with a plain copy, which lands a 47-byte stub where the 159KB attribute definitions belong, because that file is produced at sync time and not shipped in the tree; the build then reported every attribute of every component as unknown. The arm now vendors with `sync_tool` and prints two controls read back from disk — the converter's hash and the size of the definitions — before it reports anything. The release's other half, rjui no longer reading a `background` inside `partialAttributes` when the declaration never had one, needed no change here: no page puts a background there, and of the site's 20 partial entries across 13 layouts, which use only `range`, `fontColor`, `underline` and `onclick`, none carries a binding.
